from karaoke_queue.data_models.exceptions import make_bad_request_room_unknown, make_bad_request_exception
...
def try_get_player_by_uuid(room_name: str, player_id: player_uuid_hex_t) -> Player:
    ...
    if player is None:
        raise make_bad_request_exception(f"Could not verify player by id: '{player_id}'")
...
def try_get_song(song_id: int) -> Song:
    ...
    if song is None:
        raise make_bad_request_exception(f"Song with id: '{song_id}' is unknown")
